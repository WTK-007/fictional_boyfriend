...
└── src/
    ...
    │       ├── payment/
    │       │   ├── checkout/route.ts  # ★ POST 创建 Creem checkout session，必须登录态
    │       │   └── return/route.ts    # Creem success_url 回跳；校验 signature 后跳 /payment/success
    │       ├── webhooks/creem/route.ts # ★ Creem webhook（HMAC-SHA256 校验 → 落 subscriptions 表）
    ...
    │   ├── creem.ts             # ★ Creem 工具：base URL/key 前缀识别 + checkout 创建 + redirect/webhook 签名校验 + 事件解析
...
数据库表（`src/db/schema.ts`）：业务表 `users / characters / conversations / messages / media_assets / api_usage_logs / user_character_state / subscriptions` + BetterAuth 表 `sessions / accounts / verifications`。
...
文档入口：<https://docs.creem.io/features/checkout/checkout-api>、<https://docs.creem.io/learn/webhooks/introduction>

**整条链路**：

1. 前端登录态下 `POST /api/payment/checkout` `{ productId }` →
   [`src/app/api/payment/checkout/route.ts`](src/app/api/payment/checkout/route.ts) 拿 BetterAuth session，调 `createCheckoutSession`，返回 `{ checkoutUrl }`
2. 前端 `window.location = checkoutUrl` 跳到 Creem checkout 页
3. 用户付完款，Creem 跳回我们设的 `success_url = ${origin}/api/payment/return?...&signature=xxx`
4. [`src/app/api/payment/return/route.ts`](src/app/api/payment/return/route.ts) 用 **API key** 当 secret 校验 redirect 签名（HMAC-SHA256，字段按字典序拼 `key=value` 用 `&` 连接，剔除 null），通过后跳 [`/payment/success`](src/app/payment/success/page.tsx)
5. 并行地，Creem 给 `/api/webhooks/creem` 发事件（`subscription.active/paid/canceled/expired/...`），[`src/app/api/webhooks/creem/route.ts`](src/app/api/webhooks/creem/route.ts) 用 **webhook secret** 校验 `creem-signature` header，调 `upsertSubscriptionFromWebhook` 落 `subscriptions` 表

**关键点**：

- 测试/生产由 `CREEM_API_KEY` 前缀自动切换（`creem_test_xxx` → `https://test-api.creem.io`，`creem_xxx` → `https://api.creem.io`），见 [`src/lib/creem.ts`](src/lib/creem.ts) 的 `detectCreemMode`
- 两个签名密钥**不同**：redirect 用 **API key**，webhook 用 **webhook secret**（dashboard → Developers → Webhook 单独生成的那个）
- Webhook 校验 **必须用 raw body**（`request.text()`），不能 `JSON.stringify(parsed)`，否则字节级不一致
- 真正"开通会员"以 webhook 为准（有重试，更可靠）；redirect 只负责给用户跳转 + 防伪
- `metadata.userId` 在 checkout 创建时塞进去，webhook 回来时用来把订阅落到具体 user.id。续期 webhook 没带 metadata 时，按 `creem_subscription_id` 找已有订阅记录的 userId 兜底
- 处理 webhook handler 失败也回 200（已落 console.error），避免 Creem 30s/1m/5m/1h 不断重试；真正想触发重试时再返 5xx
- 是否会员的判定函数：`hasActiveSubscription(userId)`（status ∈ {trialing, active, scheduled_cancel} 且未过 `current_period_end`），在 `src/db/repo.ts`
- **前端集成（待补）**：付费按钮要 fetch `/api/payment/checkout` + 跳 `checkoutUrl`；`productId` 目前由调用方传，建议前端配置在常量文件里
...
| `src/lib/creem.ts` | redirect 签名用 **API key**、webhook 签名用 **webhook secret**，两套密钥不要混。`getEventObject` 兼容 `object`/`data` 两种信封是因为 Creem 文档里两种都出现过——别擅自只留一种。 |
| `src/app/api/webhooks/creem/route.ts` | 必须用 `request.text()` 读 raw body 验签，**不要** `request.json()` 后再 `JSON.stringify`，字节级不一致会让所有 webhook 校验失败。 |
...
| `CREEM_API_KEY` | 启用付费必填 | Creem API key（`creem_test_xxx` 测试 / `creem_xxx` 生产，前缀自动切 base URL）；同时用来验 success_url 回跳签名 |
| `CREEM_WEBHOOK_SECRET` | 启用付费必填 | Creem dashboard → Developers → Webhook 单独生成的密钥；缺省时 `/api/webhooks/creem` POST 返 500 |
...
**最后更新**：2026-05-28（接入 Creem 订阅支付：新增 `subscriptions` 表 + `src/lib/creem.ts` 工具 + `/api/payment/checkout`、`/api/payment/return`、`/api/webhooks/creem` 三个路由 + `/payment/success`、`/payment/cancel` 两个页面；测试/生产由 `CREEM_API_KEY` 前缀自动切换。env 新增 `CREEM_API_KEY`、`CREEM_WEBHOOK_SECRET`。schema 改动需跑 `pnpm db:push`）
